import firebase, { auth, db, storage } from '@/lib/firebase/client';
import {
  ContentBaseModel,
  CourseBaseModel,
  UserBaseModel,
...
export const createCourse = async ({ title, category, uid: creatorId }) => {
  const {
...
export const subscribeUserToCourse = async ({ courseUid, userUid }) => {
  const ref = db.collection('courses').doc(courseUid);
